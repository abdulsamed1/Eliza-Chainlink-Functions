...
import {
  ConfigValidator,
  GiftContractConfigFactory,
} from "../config/GiftContractConfig.ts";
...
export class GiftRequestOrchestrator {
  ...
  async processGiftRequest(params: GetGiftParams): Promise<Transaction> {
    console.log(
      `Processing gift request - Code: ${params.code}, Address: ${params.address}`
    );
...
export class GiftRequestOrchestratorFactory {
  ...
  ): Promise<GiftRequestOrchestrator> {
